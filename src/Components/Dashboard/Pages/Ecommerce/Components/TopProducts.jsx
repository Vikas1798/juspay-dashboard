...
const TopProducts = () => {
    return (
        <main className='grid gap-5 grid-cols-1 md:grid-cols-4'>
            <div className='md:col-span-3 bg-[#F7F9FB] rounded-2xl p-5 h-fit'>
                <h6 className='text-sm font-semibold'>Top Selling Products</h6>
                    ...
                    <tbody>
                        {
                            productsTableData?.map((d, i) => (
                                <tr key={i}>
                                    <td className='text-start font-normal text-[#1C1C1C] text-xs py-2'>{d?.name}</td>
                                    <td className='text-start font-normal text-[#1C1C1C] text-xs py-2'>{d?.price}</td>
                                    <td className='text-start font-normal text-[#1C1C1C] text-xs py-2'>{d?.quantity}</td>
                                    <td className='text-start font-normal text-[#1C1C1C] text-xs py-2'>{d?.amount}</td>
                                </tr>
                            ))
                        }
                    </tbody>
            ...
            <div className='md:col-span-1 bg-[#F7F9FB] rounded-2xl p-5 h-fit'>
                <h6 className='text-sm font-semibold'>Total Sales</h6>
                <div className='mt-2'>
                        ...
                        totalSales?.map((d, i) => (
                            <div className='flex justify-between items-center py-2 last:pb-0' key={i}>
                                <div className='flex items-center'>
                                    ...
                                    <p className='text-xs font-normal text-[#1C1C1C]'>{d?.by}</p>
                                </div>
                                <p className='text-xs font-normal text-[#1C1C1C]'>{d?.price}</p>
                            </div>
                        ))
                    }
                </div>
